...
func TestCompileArrayAndHashLiterals(t *testing.T) {
    c, err := Compile("nums = [1, 2, 3]\nputs nums[0]\nh = { \"a\" => 1, \"b\" => 2 }\nputs h[\"a\"]\n", "containers.rb")
    if err != nil {
        t.Fatal(err)
    }
    for _, want := range []string{"array_of(num(3)", "hash_of(num(2)", "index(nums,num(0))", "index(h,str(\"a\"))"} {
        if !strings.Contains(c, want) {
            t.Errorf("generated C missing %q", want)
        }
    }
